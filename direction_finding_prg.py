...
# output --> W     (i.e. west)
print("code find the direction North(N),South(S),West(W) and East(E) with intial direction as North(N) signified by variable in_dir we provide direcion from as Left->L and Right->R from variable ins_g to go left or right eg(with north as initial direction) input --> LLR   (in UPPER case only) output --> W     (i.e. west)")
